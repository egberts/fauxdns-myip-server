Design for a DNS authoritative server with recursor disabled
that can reply all DNS Queries with just an 'A' record of their 
client's own IP source address.

To be able to run your own myip.<domain>.tld DNS server, the prerequisites are:

- Requires a host with available port 53/tcp and 53/udp.
  - Cannot share this daemon with another authoritative DNS server
    - cannot share port 53.
- 

Daniel Perna
----
Source: https://github.com/danielperna84/myip-fakedns.git
```python
#!/usr/bin/python
import socket
import struct
